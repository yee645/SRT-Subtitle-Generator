...
# v2.3.5：最後一顆轉址鈕（清單編輯列〔字幕健檢〕）也依 D-3 移除，轉址期
# 全部走完。原本的「至少還留著一則」改成「一則都不剩」——轉址訊息是過渡
# 期的產物，留著就代表還有一顆只會切頁籤的按鈕。
check("「已整併至健檢中心」轉址訊息已經一則都不剩（v2.3.5 移除最後一顆"
      "〔字幕健檢〕轉址鈕；工具列上的兩顆在 v1.51.0 已移除）",
      app_src.count("已整併至健檢中心") == 0)
