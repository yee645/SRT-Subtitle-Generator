...
3. 同時支援兩種模式：
   - 語音轉寫以 Whisper 逐字時間軸組成字幕（build_cues_from_words）。
   - 文字稿對齊以純文字稿切行（split_into_lines），再由 aligner 配上時間。
...
# ---------------------------------------------------------------------------
# 由逐字時間軸組字幕：build_cues_from_words（語音轉寫使用）
# ---------------------------------------------------------------------------
...
def _normalize_no_overlap(cues):
    ...
    文字稿對齊常見兩句幾乎同時起始（甚至起始時間完全相同）的情況；
