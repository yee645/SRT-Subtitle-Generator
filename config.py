...
DEFAULT_CONFIG = {
    ...
    # 短片選段規劃：把一支長片自動挑成多支可直接發的直式短片。
    "clipplan": {
        "count": 3,              # 一支長片規劃出幾支短片
        "min_seconds": 15.0,     # 短片最短長度（秒）
        "max_seconds": 60.0,     # 短片最長長度（秒，Shorts 平台安全值）
        "pad_seconds": 0.4,      # 前後各留的呼吸秒數
        "min_gap_seconds": 5.0,  # 兩支短片之間的最小間隔（避免內容重疊）
    },
    ...
    "sponsorcheck": {
        ...
        "max_ratio": 0.25,               # 工商段落佔全片比例上限
        "extra_disclosure_terms": "",    # 自訂揭露詞
        "extra_promo_terms": "",         # 自訂推銷詞
        "ignore_terms": "",              # 誤判排除詞
    },
    ...
    "pacing": {
        ...
        "max_static_seconds": 25.0,   # 單一畫面超過此秒數就提醒
    },
    ...
    "subsync": {
        ...
        "min_gain": 0.10,      # 校正後至少要提升這麼多才建議套用
        "drift_gap": 0.15,     # 頭尾貼合度落差超過此值即懷疑逐漸漂移
    },
    # 廣告友善度自查（黃標風險預檢）：掃描逐字稿找可能觸發審查的用詞。
    "adfriendly": {
        "window_seconds": 30.0,    # 叢集分析的時間窗長度（秒）
        "cluster_threshold": 3.0,  # 窗內加權分數達此值標為高風險段落
        "opening_seconds": 7.0,    # 開頭區間長度（秒），此區間命中另外提醒
        "extra_terms": "",         # 使用者自訂補充詞
        "ignore_terms": "",        # 使用者自訂排除詞（頻道題材的誤判詞）
    },
    # 全片停頓自動跳剪（依字幕 cue 時間軸找出句間停頓一次剪掉）。
    "jumpcut": {
        "min_gap": 1.2,        # 句間停頓達此秒數才視為可跳剪的「冷場」
        "pad": 0.15,           # 剪點兩側各保留的緩衝秒數
        "max_cut_ratio": 0.6,  # 跳剪總長度超過影片長度此比例時中止（防誤判）
    },
    # 重複片段（NG 重錄）偵測：同一時間窗內文字高度相似的句子視為重講。
    "retakes": {
        "similarity_threshold": 0.72,  # 兩句文字相似度達此值才視為同句重講
